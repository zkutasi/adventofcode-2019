...
height = 50
width = 50
tractorbeam = [ [ None for j in range(width) ] for i in range(height) ]
num_pull = 0
...
for i in range(height):
  for j in range(width):
    ...
    tractorbeam[i][j] = '#' if result == 1 else '.'
    num_pull += result
    robot.io.print_traktor_beam(tractorbeam)

print 'Number of pulling areas: %s' % num_pull