...
class TractorBeamIO(object):
  ...
  def print_traktor_beam(self, beammap, height=50, width=50):
    print "   %s" % ''.join([ str(s / 10) if s>10 else ' ' for s in range(self.width) ])
    print "   %s" % ''.join([ str(s % 10) for s in range(self.width) ])
    for x in range(self.height):
      print "%2d %s" % (x, ''.join([ beammap[(x,y)] if (x,y) in beammap else '?' for y in range(self.width) ]))
    print
...
class Computer(object):
  def __init__(self, height, width):
    self.io = TractorBeamIO(height, width)
...
def runprogram4point(tractorbeammap, i, j):
  height = 60
  width = 60
  robot = Computer(height, width)
  robot.io.inputqueue.put(i)
  robot.io.inputqueue.put(j)
  robot.io.outputqueue = Queue()
  robot.run_program()
  result = robot.io.outputqueue.get()
  tractorbeammap[(i,j)] = '#' if result == 1 else '.'
  #robot.io.print_traktor_beam(tractorbeammap)
  return result

def look4ship(shipsize):
  tractorbeammap = {}
  initial_area = [ (a,b) for a in range(16) for b in range(16) ]
  for p in initial_area:
    i, j = p
    runprogram4point(tractorbeammap, i, j)

  open_list = sorted([ (pi, pj) for pi, pj in tractorbeammap if tractorbeammap[(pi, pj)] == '#' ])[-1:]
  while open_list:
    p = open_list.pop(0)
    i, j = p

    c0 = (i, j-shipsize+1)
    c1 = (i+shipsize-1, j-shipsize+1)
    if c1[1] >= 0 and c1 not in tractorbeammap:
      runprogram4point(tractorbeammap, c1[0], c1[1])
    if c1[1] >= 0 and tractorbeammap[c1] == '#':
      return c0

    neighbors = [ (i, j+1), (i+1, j) ]
    for n in neighbors:
      ni, nj = n
      if (ni, nj) not in tractorbeammap:
        runprogram4point(tractorbeammap, ni, nj)

    if tractorbeammap[(i, j+1)] == '#':
      open_list += [ (i, j+1) ]
    else:
      open_list += [ (i+1, j) ]

shipsize = int(sys.argv[2])
factory = InstructionFactory()
shipcorner = look4ship(shipsize)
print "Found the ship's position: %s" % str(shipcorner)
print 'Answer is: %s' % (int(shipcorner[0])*10000 + int(shipcorner[1]))
